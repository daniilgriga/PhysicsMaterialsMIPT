...
\item Точки ложатся на прямую, которая из теории должна проходить через начало координат:
$$D = k_D/P_{atm} = \frac{k_D}{757 \ \text{торр}}$$
Таким образом, МНК в предположении $y = kx$:

$k_D = 497 \pm 12\text{см}^2/\text{с}*\text{торр}$ \newline

При атмосферном давлении $P_{atm} = 757 торр$:

$D = 0.66 \pm 0.02 \text{см} ^2/\text{с} $


\item Сравним результат с табличным: при $t = 0^o C$ $D_T = 0.62 \text{см} ^2/\text{с} $ \newline
Измерения дают достаточно близкие результаты. Отклонение связано как с несовершенством теории, так и малым количеством экспериментальных точек.
